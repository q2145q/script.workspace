...
export const rewriteRouter = createTRPCRouter({
  ...
  format: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Format always uses OpenAI gpt-5-nano for speed and cost efficiency
      let resolved;
      try {
        resolved = await resolveApiKey(getSecret(), "openai", "gpt-5-nano");
      } catch {
      ...
      try {
        const result = await provider.format(
          ...
          { apiKey: resolved.apiKey, model: "gpt-5-nano" }
        );
        ...
        handleAIError(error, "Format", {
          userId: ctx.user.id, projectId: document.project.id,
          provider: "openai", model: "gpt-5-nano", feature: "format",
        });
      }
    }),
