#!/usr/bin/env npx tsx
/**
 * Structure Analysis Benchmark — Tests dramaturgical structure analysis across all models × 2 modes.
 * Short: scenes 1–5 (~8000 chars) — single-pass structure analysis
 * Long: full screenplay (~105K chars) — map-reduce (3 chunks → reduce)
 *
 * Run:
 *   cd packages/ai && npx tsx src/__tests__/structure-analysis-benchmark.ts
 */
...
const OUTPUT_MD = resolve(AI_DIR, "structure_analysis_result.md");
...
const TEST_CONFIGS: TestConfig[] = [
  {
    label: "Анализ структуры 1 — Короткий (сцены 1–5, single-pass)",
    description: `Сцены 1–5 (${shortText.length} символов) — анализ структуры за один вызов`,
    mode: "single",
    text: shortText,
  },
  {
    label: "Анализ структуры 2 — Полный сценарий (map-reduce)",
...
async function runSingle(modelDef: ModelDef, sceneText: string): Promise<BenchResult> {
  ...
  try {
    ...
    const systemPrompt = composePrompt(modelDef.provider, "structure-analysis", {
...
async function runMapReduce(modelDef: ModelDef, projectText: string): Promise<BenchResult> {
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const systemPrompt = composePrompt(modelDef.provider, "structure-analysis-map", {
    ...
    const reduceSystemPrompt = composePrompt(modelDef.provider, "structure-analysis-reduce", {
    ...
    const reduceResult = await completeAI(
      ...
      "Объедини анализ структуры из фрагментов",
      { apiKey, model: modelDef.id },
      { jsonMode: true },
    );
...
function buildMarkdown(allResults: Map<string, BenchResult[]>, csvNames: string[]): string {
  const lines: string[] = [];
  lines.push("# Structure Analysis Benchmark Results — Мурзилка");
  lines.push("");
  ...
  for (const cfg of TEST_CONFIGS) {
    ...
    lines.push("");
    lines.push(`**Режим:** ${cfg.mode === "single" ? "Один вызов (structure-analysis.md, JSON)" : "Map-Reduce (3 чанка → structure-analysis-map.md → structure-analysis-reduce.md, JSON)"}`);
    lines.push("");
...
async function main() {
  console.log("╔══════════════════════════════════════════════════════════════════╗");
  console.log("║   Structure Analysis Benchmark — Мурзилка × All Models         ║");
  console.log("╚══════════════════════════════════════════════════════════════════╝");
  ...
  for (let ti = 0; ti < TEST_CONFIGS.length; ti++) {
    ...
    const csvName = `structure_analysis_text${ti + 1}.csv`;
