#!/usr/bin/env npx tsx
/**
 * Synopsis Benchmark — Tests synopsis generation across all models × 2 modes.
 * Short: scenes 1–5 (~8000 chars) — single-pass synopsis
 * Long: full screenplay (~105K chars) — map-reduce (3 chunks → reduce)
 *
 * Run:
 *   cd packages/ai && npx tsx src/__tests__/synopsis-benchmark.ts
 */
...
const OUTPUT_MD = resolve(AI_DIR, "synopsis_result.md");
...
const TEST_CONFIGS: TestConfig[] = [
  {
    label: "Синопсис 1 — Короткий (сцены 1–5, single-pass)",
    description: `Сцены 1–5 (${shortText.length} символов) — генерация синопсиса за один вызов`,
    mode: "single",
    text: shortText,
  },
  {
    label: "Синопсис 2 — Полный сценарий (map-reduce)",
...
async function runSynopsisSingle(modelDef: ModelDef, projectText: string): Promise<BenchResult> {
  const start = Date.now();
  try {
    const apiKey = getApiKey(modelDef.provider);
    const systemPrompt = composePrompt(modelDef.provider, "synopsis", {
      USER_LANGUAGE: "ru",
    });
...
async function runSynopsisMapReduce(modelDef: ModelDef, projectText: string): Promise<BenchResult> {
  const start = Date.now();
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const systemPrompt = composePrompt(modelDef.provider, "synopsis-map", {
    ...
    // REDUCE phase
    const combinedMapResults = mapResults.map((r, i) => `--- Фрагмент ${i + 1} ---\n${r}`).join("\n\n");
    const reduceSystemPrompt = composePrompt(modelDef.provider, "synopsis-reduce", {
    ...
    const reduceResult = await completeAI(
      modelDef.provider,
      reduceSystemPrompt,
      "Объедини фрагменты в единый синопсис",
      { apiKey, model: modelDef.id },
    );
...
function buildMarkdown(allResults: Map<string, BenchResult[]>, csvNames: string[]): string {
  const lines: string[] = [];
  lines.push("# Synopsis Benchmark Results — Мурзилка");
  lines.push("");
  ...
  for (const cfg of TEST_CONFIGS) {
    ...
    lines.push("");
    lines.push(`**Режим:** ${cfg.mode === "single" ? "Один вызов (synopsis.md)" : "Map-Reduce (3 чанка → synopsis-map.md → synopsis-reduce.md)"}`);
    lines.push("");
...
async function main() {
  console.log("╔═══════════════════════════════════════════════════════╗");
  console.log("║   Synopsis Benchmark — Мурзилка × All Models         ║");
  console.log("╚═══════════════════════════════════════════════════════╝");
  ...
  for (let ti = 0; ti < TEST_CONFIGS.length; ti++) {
    ...
    const csvName = `synopsis_text${ti + 1}.csv`;
    ...
    for (const model of ALL_MODELS) {
      ...
      const result = cfg.mode === "single"
        ? await runSynopsisSingle(model, cfg.text)
        : await runSynopsisMapReduce(model, cfg.text);
