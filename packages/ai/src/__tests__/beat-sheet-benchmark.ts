#!/usr/bin/env npx tsx
/**
 * Beat Sheet Benchmark — Tests Save the Cat beat sheet generation across all models × 2 modes.
 * Short: scenes 1–5 (~8000 chars) — single-pass beat sheet
 * Long: full screenplay (~105K chars) — map-reduce (3 chunks → reduce)
 *
 * Run:
 *   cd packages/ai && npx tsx src/__tests__/beat-sheet-benchmark.ts
 */
...
const OUTPUT_MD = resolve(AI_DIR, "beat_sheet_result.md");
...
const TEST_CONFIGS: TestConfig[] = [
  {
    label: "Beat Sheet 1 — Короткий (сцены 1–5, single-pass)",
    description: `Сцены 1–5 (${shortText.length} символов) — генерация beat sheet за один вызов`,
    mode: "single",
    text: shortText,
  },
  {
    label: "Beat Sheet 2 — Полный сценарий (map-reduce)",
...
async function runBeatSheetSingle(modelDef: ModelDef, screenplayText: string): Promise<BenchResult> {
  const start = Date.now();
  try {
    const apiKey = getApiKey(modelDef.provider);
    const systemPrompt = composePrompt(modelDef.provider, "beat-sheet", {
      USER_LANGUAGE: "ru",
    });
...
async function runBeatSheetMapReduce(modelDef: ModelDef, screenplayText: string): Promise<BenchResult> {
  const start = Date.now();
  let totalIn = 0, totalOut = 0;
  try {
    const apiKey = getApiKey(modelDef.provider);
    const chunks = splitIntoChunks(screenplayText, 3);

    // MAP phase
    const mapResults: string[] = [];
    for (let i = 0; i < chunks.length; i++) {
      const chunkRange = `chunk ${i + 1}/${chunks.length} (${chunks[i].length} chars)`;
      const systemPrompt = composePrompt(modelDef.provider, "beat-sheet-map", {
    ...
    const reduceSystemPrompt = composePrompt(modelDef.provider, "beat-sheet-reduce", {
    ...
    const reduceResult = await completeAI(
      modelDef.provider,
      reduceSystemPrompt,
      "Собери beat sheet из фрагментов",
      { apiKey, model: modelDef.id },
      { jsonMode: true },
    );
...
function buildMarkdown(allResults: Map<string, BenchResult[]>, csvNames: string[]): string {
  const lines: string[] = [];
  lines.push("# Beat Sheet Benchmark Results — Мурзилка");
  lines.push("");
  ...
  for (const cfg of TEST_CONFIGS) {
    ...
    lines.push("");
    lines.push(`**Режим:** ${cfg.mode === "single" ? "Один вызов (beat-sheet.md, JSON)" : "Map-Reduce (3 чанка → beat-sheet-map.md → beat-sheet-reduce.md, JSON)"}`);
    lines.push("");
...
async function main() {
  console.log("╔═══════════════════════════════════════════════════════╗");
  console.log("║   Beat Sheet Benchmark — Мурзилка × All Models       ║");
  console.log("╚═══════════════════════════════════════════════════════╝");
  ...
  for (let ti = 0; ti < TEST_CONFIGS.length; ti++) {
    ...
    const csvName = `beat_sheet_text${ti + 1}.csv`;
    ...
    for (const model of ALL_MODELS) {
      ...
      const result = cfg.mode === "single"
        ? await runBeatSheetSingle(model, cfg.text)
        : await runBeatSheetMapReduce(model, cfg.text);
