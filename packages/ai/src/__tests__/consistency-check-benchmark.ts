#!/usr/bin/env npx tsx
/**
 * Consistency Check Benchmark — Tests consistency checking across all models x 2 modes.
 * Short: scenes 1-5 (~8000 chars) — single-pass consistency check
 * Long: full screenplay (~105K chars) — map-reduce (3 chunks -> reduce)
 *
 * Run:
 *   cd packages/ai && npx tsx src/__tests__/consistency-check-benchmark.ts
 */
...
const OUTPUT_MD = resolve(AI_DIR, "consistency_check_result.md");
...
const TEST_CONFIGS: TestConfig[] = [
  {
    label: "Проверка консистентности 1 — Короткий (сцены 1–5, single-pass)",
    description: `Сцены 1–5 (${shortText.length} символов) — проверка консистентности за один вызов`,
    mode: "single",
    text: shortText,
  },
  {
    label: "Проверка консистентности 2 — Полный сценарий (map-reduce)",
...
async function runSingle(modelDef: ModelDef, sceneText: string): Promise<BenchResult> {
  ...
  try {
    ...
    const systemPrompt = composePrompt(modelDef.provider, "consistency-check", {
      USER_LANGUAGE: "ru",
    });
...
async function runMapReduce(modelDef: ModelDef, projectText: string): Promise<BenchResult> {
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const systemPrompt = composePrompt(modelDef.provider, "consistency-check-map", {
    ...
    const reduceSystemPrompt = composePrompt(modelDef.provider, "consistency-check-reduce", {
    ...
    const reduceResult = await completeAI(
      modelDef.provider,
      reduceSystemPrompt,
      "Объедини проверку консистентности из фрагментов",
      { apiKey, model: modelDef.id },
      { jsonMode: true },
    );
...
function buildMarkdown(allResults: Map<string, BenchResult[]>, csvNames: string[]): string {
  const lines: string[] = [];
  lines.push("# Consistency Check Benchmark Results — Мурзилка");
  lines.push("");
  ...
  for (const cfg of TEST_CONFIGS) {
    ...
    lines.push("");
    lines.push(`**Режим:** ${cfg.mode === "single" ? "Один вызов (consistency-check.md, JSON)" : "Map-Reduce (3 чанка → consistency-check-map.md → consistency-check-reduce.md, JSON)"}`);
    lines.push("");
...
async function main() {
  console.log("╔══════════════════════════════════════════════════════════════════╗");
  console.log("║   Consistency Check Benchmark — Мурзилка × All Models          ║");
  console.log("╚══════════════════════════════════════════════════════════════════╝");
  ...
  for (let ti = 0; ti < TEST_CONFIGS.length; ti++) {
    ...
    const csvName = `consistency_check_text${ti + 1}.csv`;
