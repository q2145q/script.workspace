#!/usr/bin/env npx tsx
/**
 * Character Analysis Benchmark — Tests character analysis across all models × 2 modes.
 * Short: scenes 1–5 (~8000 chars) — single-pass character analysis
 * Long: full screenplay (~105K chars) — map-reduce (3 chunks → reduce)
 *
 * Run:
 *   cd packages/ai && npx tsx src/__tests__/character-analysis-benchmark.ts
 */
...
const OUTPUT_MD = resolve(AI_DIR, "character_analysis_result.md");
...
const TEST_CONFIGS: TestConfig[] = [
  {
    label: "Анализ персонажей 1 — Короткий (сцены 1–5, single-pass)",
    description: `Сцены 1–5 (${shortText.length} символов) — анализ персонажей за один вызов`,
    mode: "single",
    text: shortText,
  },
  {
    label: "Анализ персонажей 2 — Полный сценарий (map-reduce)",
...
async function runSingle(modelDef: ModelDef, sceneText: string): Promise<BenchResult> {
  ...
  try {
    ...
    const systemPrompt = composePrompt(modelDef.provider, "character-analysis", {
...
async function runMapReduce(modelDef: ModelDef, projectText: string): Promise<BenchResult> {
  ...
  try {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const systemPrompt = composePrompt(modelDef.provider, "character-analysis-map", {
    ...
    const reduceSystemPrompt = composePrompt(modelDef.provider, "character-analysis-reduce", {
    ...
    const reduceResult = await completeAI(
      modelDef.provider,
      reduceSystemPrompt,
      "Объедини анализ персонажей из фрагментов",
      { apiKey, model: modelDef.id },
      { jsonMode: true },
    );
...
function buildMarkdown(allResults: Map<string, BenchResult[]>, csvNames: string[]): string {
  const lines: string[] = [];
  lines.push("# Character Analysis Benchmark Results — Мурзилка");
  lines.push("");
  ...
  for (const cfg of TEST_CONFIGS) {
    ...
    lines.push("");
    lines.push(`**Режим:** ${cfg.mode === "single" ? "Один вызов (character-analysis.md, JSON)" : "Map-Reduce (3 чанка → character-analysis-map.md → character-analysis-reduce.md, JSON)"}`);
    lines.push("");
...
async function main() {
  console.log("╔══════════════════════════════════════════════════════════════╗");
  console.log("║   Character Analysis Benchmark — Мурзилка × All Models     ║");
  console.log("╚══════════════════════════════════════════════════════════════╝");
  ...
  for (let ti = 0; ti < TEST_CONFIGS.length; ti++) {
    ...
    const csvName = `character_analysis_text${ti + 1}.csv`;
